...
import com.caelestia.core.claim.ClaimSelection
import com.caelestia.core.visuals.MessageUtil
import com.caelestia.core.bridge.BridgeMessage
import com.caelestia.core.bridge.Source
import com.caelestia.core.bridge.Type
...
object EventHandlers {
    ...
    fun isClaimWand(stack: ItemStack): Boolean {
        if (stack.item != Items.GOLDEN_SHOVEL) return false
    ...
    fun onServerChat(event: net.neoforged.neoforge.event.ServerChatEvent) {
        ...
        val rawMessage = event.message.string
        val formattedMessage = EmojiRegistry.translateDiscordToMc(rawMessage)

        if (formattedMessage != rawMessage) {
            event.message = net.minecraft.network.chat.Component.literal(formattedMessage)
        }
        
        // Send RAW message over bridge so Paper can translate appropriately for each target
        // (unicode for in-game display, <:name:id> for Discord)
        ...
        if (serverPlayer != null) {
            Thread {
                ...
                serverPlayer.server.execute {
                    val url = CaelestiaProtection.packUrl
                    println("[Caelestia] Attempting to send resource pack prompt to ${serverPlayer.name.string}. URL: $url")
                    if (url != null) {
                        val prompt = net.minecraft.network.chat.Component.literal("§d✨ §fPlease accept the custom resource pack to see Discord emojis in chat!")
                        serverPlayer.connection.send(
                            net.minecraft.network.protocol.common.ClientboundResourcePackPushPacket(
                                java.util.UUID.randomUUID(), url, "", false, java.util.Optional.of(prompt)
                            )
                        )
                        println("[Caelestia] Pack packet sent to ${serverPlayer.name.string}!")
                    } else {
                        println("[Caelestia] Pack URL is null, skipping.")
                    }
                }
            }.start()
        }
    ...
    fun onServerStarted(event: net.neoforged.neoforge.event.server.ServerStartedEvent) {
        val bridgeMsg = BridgeMessage(
            ...
            message = "🟢 **Modded Server has started!**"
        )
        CaelestiaProtection.bridgeClient?.broadcast(bridgeMsg)
    }

    @SubscribeEvent
    fun onServerStopping(event: net.neoforged.neoforge.event.server.ServerStoppingEvent) {
        val bridgeMsg = BridgeMessage(
            source = Source.NEOFORGE,
            type = Type.SYSTEM,
            playerName = "Server",
            playerUuid = java.util.UUID.randomUUID().toString(),
            message = "🔴 **Modded Server is shutting down...**"
        )
        CaelestiaProtection.bridgeClient?.broadcast(bridgeMsg)
    }
    ...
    fun onRegisterCommands(event: RegisterCommandsEvent) {
        ...
        dispatcher.register(
            Commands.literal("emojis")
                .executes { context ->
                    ...
                    val map = EmojiRegistry.discordToMcMap
                    if (map.isEmpty()) {
                        player.sendSystemMessage(net.minecraft.network.chat.Component.literal("§cNo custom emojis have been synced yet!"))
                        return@executes 1
                    }

                    player.sendSystemMessage(net.minecraft.network.chat.Component.literal("§d✨ §f§lAvailable Custom Emojis:"))
                    
                    var currentLine = ""
                    var count = 0
                    
                    map.forEach { (name, unicode) ->
                        val formatted = "§7:$name: §f$unicode"
                        currentLine += if (currentLine.isEmpty()) formatted else "  §8|§r  $formatted"
                        
                        count++
                        if (count % 3 == 0) {
                            player.sendSystemMessage(net.minecraft.network.chat.Component.literal(currentLine))
                            currentLine = ""
                        }
                    }
        ...
        dispatcher.register(
            Commands.literal("caelestiaclaim").executes { context ->
                ...
                val wand = ItemStack(Items.GOLDEN_SHOVEL)

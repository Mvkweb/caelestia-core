...
import com.caelestia.core.CaelestiaProtection
import com.caelestia.core.emoji.EmojiRegistry
...
class BridgeClient {
    ...
    private fun processMessage(json: String) {
        try {
            ...
            if (msg.type == Type.PACK_URL) {
                CaelestiaProtection.packUrl = msg.message
                println("[Caelestia] Received pack URL from Paper: ${msg.message}")
                val currentServer = ServerLifecycleHooks.getCurrentServer()
                if (currentServer != null) {
                    Thread {
                        Thread.sleep(3500)
                        currentServer.execute {
                            val prompt = Component.literal("§d✨ §fPlease accept the custom resource pack to see Discord emojis in chat!")
                            for (player in currentServer.playerList.players) {
                                println("[Caelestia] Global broadcast: Sending resource pack prompt to ${player.name.string}. URL: ${msg.message}")
                                player.connection.send(
                                    net.minecraft.network.protocol.common.ClientboundResourcePackPushPacket(
                                        java.util.UUID.randomUUID(), msg.message, "", false, java.util.Optional.of(prompt)
                                    )
                                )
                            }
                        }
            ...
            if (msg.type == Type.EMOJI_SYNC) {
                try {
                    val mapType = object : com.google.gson.reflect.TypeToken<Map<String, String>>() {}.type
                    val map: Map<String, String> = gson.fromJson(msg.message, mapType)
                    EmojiRegistry.discordToMcMap.clear()
                    EmojiRegistry.discordToMcMap.putAll(map)
                    println("[Caelestia] Received ${map.size} emoji mappings from Paper!")
                } catch (e: Exception) {
                    println("[Caelestia] Failed to parse EMOJI_SYNC message: ${e.message}")
                }
                return
            }
            ...
            if (msg.source == Source.PAPER || msg.source == Source.DISCORD) {
                ...
                if (server != null) {
                    if (msg.type == Type.CHAT) {
                        // Format prefix - use :discord: emoji from resource pack if available (same as Paper)
                        val prefix = if (msg.source == Source.DISCORD) {
                            val discordIcon = EmojiRegistry.discordToMcMap["discord"] ?: "💬"
                            Component.literal("$discordIcon ").withStyle(ChatFormatting.WHITE)
                                .append(Component.literal("${msg.playerName} ").withStyle(ChatFormatting.GRAY))
                                .append(Component.literal("• ").withStyle(ChatFormatting.DARK_GRAY))
                        } else {
                            Component.literal("§b${msg.playerName} §8» §f")
                        }
                        val translatedContent = EmojiRegistry.translateDiscordToMc(msg.message)
                        var finalMsg = prefix.copy().append(Component.literal(translatedContent.replace("<gold>", "§6").replace("</gold>", "§f")).withStyle(ChatFormatting.WHITE))
                    ...
                    } else if (msg.type == Type.DISCORD_REPLY) {
                        val parts = msg.message.split("\n", limit = 2)
                        val replyContext = if (parts.size > 1) parts[0] else ""
                        val replyMessage = if (parts.size > 1) parts[1] else msg.message
                        
                        val discordIcon = EmojiRegistry.discordToMcMap["discord"] ?: "💬"
                        
                        val translatedContent = EmojiRegistry.translateDiscordToMc(replyMessage)
                        
                        var author = ""
                        if (replyContext.isNotEmpty()) {
                            val colonIdx = replyContext.indexOf(": ")
                            author = if (colonIdx != -1) {
                                replyContext.substring(0, colonIdx)
                            } else {
                                replyContext
                            }
                        }

                        var chatLine: MutableComponent = Component.literal("$discordIcon ").withStyle(ChatFormatting.WHITE)
                        
                        if (author.isNotEmpty()) {
                            chatLine = chatLine.append(Component.literal("[Replying to $author] ").withStyle(ChatFormatting.DARK_GRAY))
                        }
                        
                        chatLine = chatLine
                            .append(Component.literal("${msg.playerName} ").withStyle(ChatFormatting.GRAY))
                            .append(Component.literal("• ").withStyle(ChatFormatting.DARK_GRAY))
                            .append(Component.literal(translatedContent).withStyle(ChatFormatting.WHITE))
                        
                        server.playerList.broadcastSystemMessage(chatLine, false)
                    } else if (msg.type == Type.JOIN) {
                        val vanillaIcon = EmojiRegistry.discordToMcMap["vanilla"] ?: EmojiRegistry.discordToMcMap["grass_block"] ?: "🌱"
                        val joinMsg = Component.literal("$vanillaIcon ").withStyle { it.withColor(TextColor.fromRgb(0x55FF55)) }
                            .append(Component.literal("${msg.playerName} ").withStyle(ChatFormatting.GRAY))
                            .append(Component.literal("joined the vanilla server").withStyle(ChatFormatting.DARK_GRAY))
                        server.playerList.broadcastSystemMessage(joinMsg, false)
                    } else if (msg.type == Type.QUIT) {
                        val vanillaIcon = EmojiRegistry.discordToMcMap["vanilla"] ?: EmojiRegistry.discordToMcMap["grass_block"] ?: "🌱"
                        val quitMsg = Component.literal("$vanillaIcon ").withStyle { it.withColor(TextColor.fromRgb(0xFF5555)) }
                            .append(Component.literal("${msg.playerName} ").withStyle(ChatFormatting.GRAY))
                            .append(Component.literal("left the vanilla server").withStyle(ChatFormatting.DARK_GRAY))
                        server.playerList.broadcastSystemMessage(quitMsg, false)
